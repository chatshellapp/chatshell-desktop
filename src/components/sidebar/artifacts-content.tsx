...
function getLanguageBadge(language?: string) {
  ...
  return (
    <span className={cn("text-[10px] px-1.5 py-0.5 rounded", colors[language] || "bg-muted text-muted-foreground")}>
      {language}
    </span>
  )
}
            ...
            {group.artifacts.map((artifact) => (
              <button
                ...
                className={cn(
                  ...
                  selectedArtifactId === artifact.id && "bg-sidebar-accent text-sidebar-accent-foreground border-primary/50"
                )}
              >
                <div className="flex w-full items-center gap-2">
                  ...
                  >
                    <Star
                      className={cn(
                        "size-3.5",
                        artifact.isStarred ? "fill-yellow-500 text-yellow-500" : "text-muted-foreground"
                      )}
                    />
                  </button>
                </div>
