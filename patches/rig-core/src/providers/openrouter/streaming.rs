        ...
        while let Some(event_result) = event_source.next().await {
            match event_result {
                ...
                Ok(Event::Message(message)) => {
                    ...
                    // Capture reasoning signatures from reasoning_details (all types).
                    // For Anthropic models via OpenRouter, signatures are needed to send
                    // thinking blocks back in multi-turn conversations.
                    for reasoning_detail in &delta.reasoning_details {
                        match reasoning_detail {
                            ReasoningDetails::Text { signature, id, .. } => {
                                if let Some(sig) = signature {
                                    let thinking = current_thinking.get_or_insert_with(ThinkingState::default);
                                    thinking.signature = Some(sig.clone());
                                    if thinking.id.is_none() {
                                        thinking.id = id.clone();
                                    }
                                }
                            }
                            ReasoningDetails::Encrypted { id, data, .. } => {
                                if let Some(id) = id {
                                    if let Some(tool_call) = tool_calls.values_mut().find(|tc| tc.id.eq(id)) {
                                        if let Ok(additional_params) = serde_json::to_value(reasoning_detail) {
                                            tool_call.signature = Some(data.clone());
                                            tool_call.additional_params = Some(additional_params);
                                        }
                                    } else {
                                        let thinking = current_thinking.get_or_insert_with(ThinkingState::default);
                                        thinking.signature = Some(data.clone());
                                        if thinking.id.is_none() {
                                            thinking.id = Some(id.clone());
                                        }
                                    }
                                }
                            }
                    ...
                    // Flush accumulated thinking as a full Reasoning block when
                    // transitioning to text content, tool calls, or stream end.
                    // This replaces the delta-accumulated item (with signature: None)
                    // with one that has the proper signature.
                    let transitioning = (delta.content.as_ref().is_some_and(|c| !c.is_empty()))
                    ...
                    if let Some(reasoning) = &delta.reasoning && !reasoning.is_empty() {
                        let thinking = current_thinking.get_or_insert_with(ThinkingState::default);
                        thinking.text.push_str(reasoning);
