        ...
        match Pin::new(&mut stream.inner).poll_next(cx) {
            ...
            Poll::Ready(Some(Ok(choice))) => match choice {
                ...
                RawStreamingChoice::Reasoning { id, content } => {
                    ...
                    // Replace the delta-accumulated item (which has signature: None)
                    // instead of pushing a duplicate, so multi-turn messages don't
                    // contain thinking blocks with invalid signatures.
                    if let Some(index) = stream.reasoning_item_index {
                        stream.assistant_items[index] =
