...
impl McpConnectionManager {
    ...
    pub async fn connect(&self, tool: &Tool) -> Result<McpServerConnection> {
        let endpoint = tool
            .endpoint
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("MCP server has no endpoint configured"))?;

        tracing::info!("🔌 Connecting to MCP server: {} at {}", tool.name, endpoint);

        // Create Streamable HTTP transport
        let http_client = reqwest::Client::new();
        let config = StreamableHttpClientTransportConfig {
            uri: endpoint.clone().into(),
            ..Default::default()
        };
        ...
        // IMPORTANT: The RunningService returned by serve() owns the background transport task.
        // We MUST keep it alive for the connection to remain active. Dropping it cancels the transport.
    ...
    pub async fn test_connection(&self, endpoint: &str) -> Result<Vec<McpTool>> {
        tracing::info!("🧪 Testing connection to MCP server at {}", endpoint);
